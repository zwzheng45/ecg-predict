...
int main2() {
    ...
    /*
    // 打开MAT文件 Open mat file
    mat_t *matfp = Mat_Open("/Users/zzw/学习/UK/Computer Science/A-level project/David A-LEVEL Project/生物传感器/生物信号处理分析算法/projects/基于心电图和机器学习的异常诊断/ecg/ecg-master/examples/cinc17/data/training2017/A06767.mat", MAT_ACC_RDONLY);
    if (matfp == NULL) {
    ...
    // 读取变量 get variables
    matvar_t *matvar = Mat_VarRead(matfp, "val");
    if (matvar == NULL) {
        cerr << "Error reading variable from MAT file!" << endl;
        Mat_Close(matfp);
        return 1;
    }

    int cols = (int)matvar->dims[0];
    int rows = (int)matvar->dims[1];
    float *data = static_cast<float*>(matvar->data);
*/
    
    //从文件读取 Read from binary file
    int cols=1,rows;
    FILE * pfile=fopen("/Users/zzw/学习/UK/Computer Science/A-level project/David A-LEVEL Project/生物传感器/生物信号处理分析算法/projects/基于心电图和机器学习的异常诊断/ecg/ecg-master/ecg/A08495.bin","rb");
    ...
    /*
    // 释放MAT变量和关闭文件 Free mat file and variables
    Mat_VarFree(matvar);
    Mat_Close(matfp);
...
//    // 创建输入Tensor Create input tensor
//    Tensor input_tensor(DT_FLOAT, TensorShape({1, STEP, 1}));
//    auto input_tensor_mapped = input_tensor.tensor<float, 3>();
//
//    // 填充输入Tensor数据（这里假设输入是全零）
//    for (int i = 0; i < 12; ++i) {
//        for (int j = 0; j < 1; ++j) {
